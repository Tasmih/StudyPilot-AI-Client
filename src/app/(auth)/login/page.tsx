...
export default function LoginPage() {
  ...
  const handleDemoLogin = () => {
    Swal.fire({
      ...
      text: "You will be logged in with a pre-configured demo account to explore StudyPilot AI.",
      icon: "info",
      showCancelButton: true,
      confirmButtonColor: "#4F46E5",
      cancelButtonColor: "#64748B",
      confirmButtonText: "Yes, login as Demo",
    }).then((result) => {
      if (result.isConfirmed) {
        setValue("email", "demo@studypilot.ai");
      ...
      <CardContent>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <Button 
            ...
            disabled={isLoading || isGoogleLoading}
          >
            {isLoading ? <Loading size="sm" className="p-0 mr-2" /> : <LogIn className="mr-2 h-4 w-4" />}
            Sign In
          </Button>
        </form>
